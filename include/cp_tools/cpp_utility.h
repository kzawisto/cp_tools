...
std::ostream & operator<<(std::ostream & op, const std::unordered_map<T, K> & vec) {
	op<<"<";
	for(const auto & t : vec){
		op<<t.first <<"->"<<t.second<<", ";
	}
...
inline
std::string token_at(const std::string & line, int i, char sep = '\001') {
	int j = i;
	for(; j < i + 20; ++j){
		if(line.at(j) == sep) {
			break;
...
template<typename T>
std::string stringifySep(std::string sep, T arg) {
...
template<typename T>
void cumsum_inplace(T begin, T end) {
	auto it = begin;
...
template<typename T>
std::string stringify_collection(T begin, T end) {
	std::stringstream ss;
	auto it = begin;
	ss<<"{";
	if(it != end) {
