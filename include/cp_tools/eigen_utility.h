...
inline void cumsum_inplace(Ei::VectorXd &v) {
...
void binary_serialize_vecd(Ei::VectorXd vec, std::string file) {
  ...
  if (f.good()) {
    f.write((const char *)vec.data(), vec.size() * 8);
    f.close();
  }
}
...
inline Ei::VectorXd random_normal(int size, int seed = 1) {
...
inline Ei::VectorXd cumsum(const Ei::VectorXd &v) {
