...
inline DateTime date_time_from_ptime(pt::ptime t) {
  DateTime dt;
...
inline lt::local_date_time
build_local_time(gr::date dt, pt::time_duration time,
                 boost::shared_ptr<boost::local_time::posix_time_zone> tz) {
  return lt::local_date_time(dt, time, tz,
                             lt::local_date_time::check_dst(dt, time, tz));
}
...
struct TimeToEpochConverter {
  ...
  std::vector<MonthId> get_month_frames(int year) {
    ...
    for (int j = 1; j <= 12; ++j) {
      ...
      long double epoch = pt::to_time_t(
          build_local_time(gr::date(year, j, 1), pt::time_duration(0, 0, 0), tz)
              .utc_time());
      cache.push_back(
          MonthId{epoch, DateTime{Date{year, j, 1}, Time{0, 0, 0}}});
    }
    return cache;
  }
  std::vector<MonthId> get_dst_frames(int year) {
    auto start_dst_local_time =
        tz->dst_local_start_time(year) + tz->dst_offset();
    auto start_dst_utc_time =
        lt::local_date_time(start_dst_local_time.date(),
                            start_dst_local_time.time_of_day(), tz, true)
            .utc_time();
    auto end_dst_local_time = tz->dst_local_end_time(year);
    auto end_dst_utc_time =
        lt::local_date_time(end_dst_local_time.date(),
                            end_dst_local_time.time_of_day(), tz, false)
            .utc_time();

    return std::vector<MonthId>{
        MonthId{to_time_t(start_dst_utc_time),
                date_time_from_ptime(start_dst_local_time)},
        MonthId{to_time_t(end_dst_utc_time),
                date_time_from_ptime(end_dst_local_time)}};
  }
  std::vector<MonthId> get_frames_per_year(int year) {
    ...
    if (tz->has_dst()) {
      ...
      frames_month.insert(frames_month.end(), frames_dst.begin(),
                          frames_dst.end());
    }
    return frames_month;
  }
  TimeToEpochConverter(boost::shared_ptr<lt::posix_time_zone> tz,
                       int year_from = 1990, int year_to = 2020)
      : tz(tz) {

    for (int i = year_from; i <= year_to; ++i) {
      auto frames = get_frames_per_year(i);
      key_frames.insert(key_frames.end(), frames.begin(), frames.end());
    }
    std::stable_sort(key_frames.begin(), key_frames.end());
  }
  DateTime get_datetime(double epoch) {
    auto it = std::lower_bound(key_frames.begin(), key_frames.end(),
                               MonthId{epoch, DateTime{}});
    it--;
  ...
  double get_epoch(DateTime dt) {
    auto it = std::lower_bound(
        ...
        [](const MonthId &a, const MonthId &b) { return a.date < b.date; });
    it--;
