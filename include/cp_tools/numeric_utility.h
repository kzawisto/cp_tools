...
template <class T> using deref_t = typename deref<T>::type;
...
template <typename T>
auto get_mean_and_variance(T begin, T end)
    -> MeanAndVariance<deref_t<decltype(*begin)>> {
...
template <typename T> class RollingMeanAndVariance {
public:
  ...
  RollingMeanAndVariance(std::initializer_list<T> initial_values) {
    mean_var =
        get_mean_and_variance(initial_values.begin(), initial_values.end());
  }
...
public:
  ...
  MeanAndVariance<T> with_removed_value(T a) const {
    if (mean_var.count <= 2) {
      if (mean_var.count == 0)
        ...
        return MeanAndVariance<T>{(T)2 * mean_var.mean - a, (T)0, 1};
      }
    } else {
      double new_mean =
          (mean_var.count * mean_var.mean - a) / (mean_var.count - (T)1.0);
      double new_variance = ((mean_var.count - T(1)) * mean_var.variance -
                             (a - new_mean) * (a - mean_var.mean)) /
                            (mean_var.count - (T)2);
  ...
  MeanAndVariance<T> with_added_value(T a) const {
    ...
    new_mean_var.mean =
        mean_var.mean + (a - mean_var.mean) / (mean_var.count + (T)1);
    new_mean_var.variance =
        mean_var.variance +
