...
TEST(rolling_mean_and_variance, should_compute_mean_and_variance_for_added_value) {
	RollingMeanAndVariance<double> mean_var({1.0, 2.0,3.0});
...
TEST(rolling_mean_and_variance, should_compute_mean_and_variance_for_removed_value) {
...
TEST(rolling_mean_and_variance, should_produce_same_result_as_two_pass_when_appending) {
...
TEST(rolling_mean_and_variance, should_produce_same_result_as_two_pass_for_rolling_forward) {
...
TEST(rolling_mean_and_variance, should_produce_same_result_as_two_pass_for_rolling_backward) {
	std::deque<double> vec;
	RollingMeanAndVariance<double> mean_var;
	srand(7);
	for(int i =0 ;i < 1000; i ++) {
