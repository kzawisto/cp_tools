...
void test_lower_bound_cache_loc_against_stl(Collection coll, T value, K guess) {
  auto actual =
      lower_bound_local(coll.begin(), coll.end(), value, coll.begin() + guess) -
      coll.begin();
  auto expected = lower_bound(coll.begin(), coll.end(), value) - coll.begin();
  ASSERT_EQ(actual, expected) << "collection:" << stringify(coll)
                              << " value: " << value << " guess:" << guess;
}
...
TEST(algo, cache_local_works_on_c_arr) {
  ...
  auto value = 5;
  auto actual = lower_bound_local(coll.data(), coll.data() + coll.size(), value,
                                  coll.data() + 4) -
                coll.data();
  auto expected =
      std::lower_bound(coll.data(), coll.data() + coll.size(), value) -
      coll.data();
